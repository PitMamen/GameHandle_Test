 ...
 */
public class SFRockerView extends SurfaceView implements Runnable, SurfaceHolder.Callback {
    ...
    public SFRockerView(Context context) {
        this(context, null);
    }

    public SFRockerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SFRockerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            if (event.getAction() == MotionEvent.ACTION_MOVE) {
                if (len <= mAreaRadius) {
                    ...
                    Log.d(getClass().getSimpleName(), mRockerPosition.toString());
                    Log.d(getClass().getSimpleName(), event.getX() + "," + event.getX());

                }
                if (mListener != null) {
                    float radian = MathUtils.getRadian(mAreaPosition, new Point((int) event.getX(), (int) event.getY()));
                    int angle = SFRockerView.this.getAngleConvert(radian);
