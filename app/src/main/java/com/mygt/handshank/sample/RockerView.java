...
public class RockerView extends View {
    ...
    private int mSelectRockerBackgroundMode = ROCKER_BACKGROUND_MODE_DEFAULT;
    private Bitmap mRockerBitmap;
    private Bitmap mSelectRockerBitmap;
    private int mRockerColor;
    private int mSelectRockerColor;
    ...
    private void initAttribute(Context context, AttributeSet attrs) {
        ...
        Log.d(TAG, "mRockerRadius:"+mRockerRadius);
        ...
        if (null != rockerBackground) {
            ...
            if (rockerBackground instanceof BitmapDrawable) {
                ...
                Log.d(TAG, "mRockerBitmap,width:"+mRockerBitmap.getWidth());
        ...
        // 选中状态摇杆背景
        Drawable selectRockerBackground = typedArray.getDrawable(R.styleable.RockerView_selectRockerBackground);
        mSelectRockerColor = Color.RED;
        if (null != selectRockerBackground) {
            // 设置了摇杆背景
            if (selectRockerBackground instanceof BitmapDrawable) {
                // 图片
                mSelectRockerBitmap = ((BitmapDrawable) selectRockerBackground).getBitmap();
                Log.d(TAG, "mSelectRockerBitmap,width:"+mSelectRockerBitmap.getWidth());
                mSelectRockerBackgroundMode = ROCKER_BACKGROUND_MODE_PIC;
            } else if (selectRockerBackground instanceof ColorDrawable) {
                // 色值
                mSelectRockerColor = ((ColorDrawable) selectRockerBackground).getColor();
                mSelectRockerBackgroundMode = ROCKER_BACKGROUND_MODE_COLOR;
            }
        }else {
            // 没有设置摇杆背景
            mSelectRockerBackgroundMode = ROCKER_BACKGROUND_MODE_DEFAULT;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 可移动区域的半径
        mAreaRadius = ((measuredWidth <= measuredHeight) ? cx : cy)-mRockerRadius;
        ...
        if (AREA_BACKGROUND_MODE_PIC == mAreaBackgroundMode || AREA_BACKGROUND_MODE_XML == mAreaBackgroundMode) {
            ...
            dst.set(mCenterPoint.x - mAreaRadius, mCenterPoint.y - mAreaRadius, mCenterPoint.x + mAreaRadius,
        ...
        // 画摇杆
        if (click) {
            if (ROCKER_BACKGROUND_MODE_PIC == mSelectRockerBackgroundMode || ROCKER_BACKGROUND_MODE_XML == mSelectRockerBackgroundMode) {
                // 图片
                src.set(0, 0, mSelectRockerBitmap.getWidth(), mSelectRockerBitmap.getHeight());
                dst.set(mRockerPosition.x - mRockerRadius, mRockerPosition.y - mRockerRadius, mRockerPosition.x + mRockerRadius,
                        mRockerPosition.y + mRockerRadius);
                canvas.drawBitmap(mSelectRockerBitmap, src, dst, mRockerPaint);
            } else if (ROCKER_BACKGROUND_MODE_COLOR == mSelectRockerBackgroundMode) {
                // 色值
                mRockerPaint.setColor(mSelectRockerColor);
    ...
    public void moveRockerWithKey(float x, float y) {
        mRockerPosition = getRockerPositionPoint(mCenterPoint, new Point((int) x, (int) y), mAreaRadius, mRockerRadius);
        moveRocker(mRockerPosition.x, mRockerPosition.y);
    }
